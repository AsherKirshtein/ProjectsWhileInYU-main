...
public class GatewayServer extends Thread
{
    ...
    protected GatewayPeerServerImpl gatewayPeerServerImpl;
    ...
    public GatewayServer(int port, Map<Long, InetSocketAddress> peerIDtoAddress)
    {
        ...
        this.gatewayPeerServerImpl = new GatewayPeerServerImpl(69, 69, 69, peerIDtoAddress);
    ...
    public void run()
    {
        this.gatewayPeerServerImpl.run();

        while(!this.isInterrupted())
        {
            this.server.start();// got the http from the client
            this.leaderId = this.gatewayPeerServerImpl.getCurrentLeader().getProposedLeaderID();
        }
    }
